...
def training_pipeline(path, model_info_db_name="models-db"):
    ...
    print(
        "------> Saving the model {} object on the cloud".format(
            "model_" + str(int(ts))
        )
    )
...
def save_model(obj, name, timestamp, bucket_name="models-fod-uem"):
    """
...
def get_best_model(model_metrics1, model_metrics2):
    ...
    print(
        "---------> TRAINED model {} with AUC score: {}".format(
            model_metrics1["_id"], str(round(auc1, 3))
        )
    )
    print(
        "---------> CURRENT model in PROD {} with AUC score: {}".format(
            model_metrics2["_id"], str(round(auc2, 3))
        )
    )
